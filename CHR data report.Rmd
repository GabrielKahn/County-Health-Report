...
# Associations of Social Factors

```{r}
ggplot(swing_states_county, aes(State.Abbreviation, Drug.overdose.deaths.raw.value, fill = State.Abbreviation))+
  geom_boxplot() +
  theme_minimal() +
   labs(x = "Swing State", y = "Drug Overdoses per 100,000", title = "Drug Overdoses in Swing States" ) +
  theme(legend.position="none")
```

```{r}
ggplot(swing_states_county, aes(State.Abbreviation, Children.in.poverty.raw.value, fill = State.Abbreviation))+
  geom_boxplot() +
  theme_minimal() +
   labs(x = "Swing State", y = "Child Poverty Rate", title = "Child Poverty in Swing States in Swing States" ) +
  theme(legend.position="none")
```